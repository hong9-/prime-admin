import { PrismaClient, Schedule, ScheduleResult } from '@prisma/client'
...
async function main() {
  ...
  // const schedulesExample = scheduleList.map((schedule,i)=>{
  ...
  //   }
  //   let addressAbstract = address.split(' ').slice(0, 2).join(' ');
  ...
  // schedulesExample.sort((scheduleA, scheduleB)=> scheduleA.date.getTime() - scheduleB.date.getTime())
  ...
  //   },
  // }> = [];
  // await schedulesExample.reduce(async(prev, current, i)=> {
  //   await prev;
  ...
  // const notifications = await schedules.filter((schedule)=>schedule.result === ScheduleResult.CONTRACT).reduce(async(prevJob, schedule, i)=> {
  //   await prevJob;
  ...
  //         }
  //       },
  //       message: `${schedule.manager.name} ${schedule.company} 계약`,
  //       link: `/ScheduleList/${schedule.id}`,
  //     }
  //   }).then((e)=> {
